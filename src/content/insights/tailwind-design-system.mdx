...
```tsx
<Button variant="primary" href="/insights" className="mt-8">
    Primary
    <ChevronRight />
</Button>
```
...
Tailwind uses literal color names (like red, green, etc.) and a numeric scale (where 50 is light and 900 is dark) by default. Read more about it [here](https://tailwindcss.com/docs/customizing-colors#naming-your-colors).

From Tailwinds creators point of view these literal color names make sense. However in my opinion when working with designers and multiple developers, it's better to use more abstract names. For example, instead of using `bg-red-500` you could use `bg-danger-500`. This makes it easier to understand the context of the color.
